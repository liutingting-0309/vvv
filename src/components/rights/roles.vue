<template>
        ...
        <el-table :data="rolesList" style="width: 100%" stripe border>
          <el-table-column type="expand">
            <template slot-scope="scope">
              <el-row v-for="(item1,idx1) in scope.row.children" :key="idx1"  :class="['bdbottom',idx1 === 0?'bdtop':'','vcenter']">
                <!-- 一级权限 -->
                <el-col :span="5">
                ...
                <!-- 二级、三级权限 -->
                <el-col :span="18">
                  <el-row v-for="(item2,idx2) in item1.children" :key="idx2" :class="[ idx2 === 0?'':'bdtop','vcenter' ]">
                    <el-col :span="7">
                      <el-tag type="warning" closable  @close="removeRightById(scope.row,item2.id)">{{ item2.authName }}</el-tag>
                      <i class=" el-icon-caret-right"></i>
                    </el-col>
                    <el-col :span="17">
                      <el-tag type="success" v-for="(item3,idx3) in item2.children" :key="idx3" closable @close="removeRightById(scope.row,item3.id)">{{item3.authName }}</el-tag>
                    </el-col>
                  </el-row>
                </el-col> 
              </el-row>
          ...
          <el-table-column prop="date" label="操作">
            <template slot-scope="scope">
              <el-button type="primary" icon="el-icon-edit"  size="mini"  @click="showeditrolesform(scope.row.id)">编辑</el-button>
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleterolelist(scope.row.id)">删除</el-button>
  ...
  <el-tree :data="setRightList"  :props="treeProps"  show-checkbox node-key="id" :default-checked-keys="defKeys" default-expand-all ref="treeRef"></el-tree>
  <span slot="footer" class="dialog-footer">
